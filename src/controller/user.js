...
exports.addSongToFavorite = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            message: 'Cập nhập danh sách yêu thích thành công',
        })
    } catch (error) {
        return res
            .status(500)
...
exports.deleteSongFromFavorite = async (req, res) => {
    const { user_id = '', song_id = '' } = req.body

    try {
        const song = await User.findOneAndUpdate(
            { _id: user_id, favorite_song:{ $in: [song_id]} },
            { $pull: { favorite_song:{ $in: [song_id] } }}
        )
        ...
        return res.status(200).json({
            success: true,
            message: 'Đã xóa bài hát khỏi danh sách yêu thích thành công',
        })
    } catch (error) {
        return res
            .status(500)
